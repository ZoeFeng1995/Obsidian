...
async function splitPieces2(s, r, o, u, g) {
  if (o || isTextBlob(s)) return splitPiecesText(await s.text(), r, o, u);
  let _ = 0;
...
function createSavingEntryFromLoadedEntry(s) {
  const r = "newnote" == s.type ? "newnote" : "plain";
  return {
    ...s,
    data: readAsBlob(s),
    datatype: r,
...
var pluginList = writable([]), pluginIsEnumerating = writable(false), ConfigSync = class extends LiveSyncCommands {
  constructor(s) {
    ...
    this.pluginScanProcessor = new QueueProcessor((async s => {
      ...
      this.pluginList = r;
      pluginList.set(r);
      if (10 != s.length) await waitForSignal("plugin-next-load", 1e3);
    }), {
      suspended: false,
      batchSize: 10,
      concurrentLimit: 2,
...
var import_obsidian5 = require("obsidian"), ObsidianLiveSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const E = r.createDiv(), w = E.createEl("h3", {
      ...
      text: ""
    }), L = "0.22.14", O = ~~(versionNumberString2Number(L) / 1e3), D = createSpan();
    ...
    const le = new import_obsidian.Setting(oe).setName("Device name").setDesc("Unique name between all synchronized devices").addText((s => {
...
async function putDBEntry(s, r) {
  ...
  const w = r.data instanceof Blob ? r.data : b ? createTextBlob(r.data) : createBinaryBlob(r.data);
  r.datatype = b ? "plain" : "newnote";
  r.type = r.datatype;
...
var SETTING_HEADER = "````yaml:livesync-setting\n", SETTING_FOOTER = "\n````", ObsidianLiveSyncPlugin = class extends import_obsidian.Plugin {
  ...
  async checkAndApplySettingFromMarkdown(s, r) {
    if (r && !this.settings.notifyAllSettingSyncFile) if (!this.settings.settingSyncFile || this.settings.settingSyncFile != s) {
      Logger(`Setting file (${s}) is not matched to the current configuration. skipped.`, LOG_LEVEL_VERBOSE);
      return;
    }
  ...
  async processEntryDoc(s, r, o) {
    ...
    if (s._deleted || s.deleted || false === _) {
      ...
      if (false === _) await this.deleteVaultItem(r); else await this.pullFile(g, null, true);
      return;
    }
    const b = compareFileFreshness(r, s), E = _;
    if ("newnote" != E.datatype && "plain" != E.datatype) {
      Logger(m + "ERROR, Invalid datatype: " + g + "(" + E.datatype + ")", LOG_LEVEL_NOTICE);
      return;
    }
  ...
  async syncAllFiles(s) {
    ...
    L.push(runAll("UPDATE STORAGE", E, (async s => {
      ...
      if (r && !(r.deleted || r._deleted)) if (!this.isFileSizeExceeded(r.size)) {
        await this.pullFile(s, o, false, null, false);
    ...
    switch (compareFileFreshness(s, r)) {
     ...
     case TARGET_IS_NEW:
      if (!this.isFileSizeExceeded(r.size)) {
        Logger("STORAGE <- DB :" + s.path);
        const r = await this.localDatabase.getDBEntry(getPathFromTFile(s), null, false, false, true);
  ...
  async updateIntoDB(s, r, o) {
    ...
    const u = isPlainText(s.name), g = !u;
    if (g) Logger(`Reading   : ${s.path}`, LOG_LEVEL_VERBOSE);
    const _ = createBlob(await this.vaultAccess.vaultReadAuto(s)), m = u ? "plain" : "newnote";
    if (g) Logger(`Processing: ${s.path}`, LOG_LEVEL_VERBOSE);
    const b = getPathFromTFile(s), E = {
    ...
    if (await serialized("file-" + b, (async () => {
      ...
      try {
        ...
        if (false !== s) {
          ...
          Logger(w + "Skipped (not changed) " + b + (E._deleted || E.deleted ? " (deleted)" : ""), LOG_LEVEL_VERBOSE);
          markChangesAreSame(s, E.mtime, s.mtime);
          return true;
        }
      } catch (s) {
        if (o) Logger(w + "Error, Could not check the diff for the old one." + (o ? "force writing." : "") + b + (E._deleted || E.deleted ? " (deleted)" : ""), LOG_LEVEL_VERBOSE); else Logger(w + "Error, Could not check the diff for the old one." + b + (E._deleted || E.deleted ? " (deleted)" : ""), LOG_LEVEL_VERBOSE);
        return !o;
      }
      return false;
    }))) {
      Logger(w + " Skip " + b, LOG_LEVEL_VERBOSE);
      return true;
    }
    const S = await this.localDatabase.putDBEntry(E);
    if (false !== S) {
      Logger(w + b);
